...
export default async function handler(
  ...
  try {
    ...
    console.log('🚀 Starting market value population script...');
    ...
    const child = spawn('npm', ['run', 'populate:market-values'], {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env },
      cwd: process.cwd()
    ...
    return response.status(200).json({
      success: true,
      message: 'Market value population started successfully',
