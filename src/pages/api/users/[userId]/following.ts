...
function extractUserIdFromPath(url: string): string | null {
  const match = url.match(/\/api\/users\/([^\/]+)\/following/);
 ...
 */
async function canViewUserFollowing(targetUserId: string, requestingUserId?: string): Promise<boolean> {
 ...
 */
async function handleGetFollowing(request: Request): Promise<Response> {
  ...
  try {
    // Check if requesting user can view this user's following list
    const canView = await canViewUserFollowing(targetUserId, user.id);
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to fetch following list' }),
    ...
    const formattedFollowing = following.map(follow => ({
      ...
      fullName: follow.following.full_name,
      profileIsPublic: follow.following.profile_is_public,
    ...
    console.error('API Error in GET /api/users/{userId}/following:', error);
    return new Response(
...
/**
 * Main API handler for /api/users/{userId}/following
 */
export async function handler(request: Request): Promise<Response> {
  try {
    ...
    console.error('API Error in /api/users/{userId}/following:', error);
    return new Response(
      JSON.stringify({ success: false, error: 'Internal server error' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
