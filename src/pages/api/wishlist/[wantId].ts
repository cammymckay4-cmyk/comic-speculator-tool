...
/**
 * Extract wantId from URL path
 */
function extractWantId(url: string): string | null {
  const urlObj = new URL(url);
  const pathParts = urlObj.pathname.split('/');
  const wantId = pathParts[pathParts.length - 1];
  
  // Basic UUID validation
  const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
  
  return uuidRegex.test(wantId) ? wantId : null;
}

/**
 * PUT /api/wishlist/{wantId} - Update wishlist item criteria
 */
async function handleUpdateWishlistItem(request: Request, wantId: string): Promise<Response> {
  ...
  try {
    const body: UpdateWishlistRequest = await request.json();
    ...
    if (body.max_price !== undefined) updateData.current_value = body.max_price;
    ...
    if (updateData.series_id || updateData.issue_id || updateData.grade_id !== undefined) {
      ...
      if (duplicate) {
        return new Response(
          JSON.stringify({ 
            success: false, 
            error: 'A comic with these details already exists in your wishlist' 
          }),
    ...
    if (error) {
      ...
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to update wishlist item' }),
...
/**
 * DELETE /api/wishlist/{wantId} - Remove comic from wishlist
 */
async function handleDeleteWishlistItem(request: Request, wantId: string): Promise<Response> {
  const { user, error: authError } = await validateAuthToken(request);
  ...
  try {
    ...
    if (error) {
      console.error('Database error:', error);
      return new Response(
        JSON.stringify({ success: false, error: 'Failed to delete wishlist item' }),
        { status: 500, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    console.error('API Error in DELETE /api/wishlist/[wantId]:', error);
    return new Response(
...
/**
 * Main API handler for /api/wishlist/[wantId]
 */
export async function handler(request: Request): Promise<Response> {
  try {
    ...
    // Extract wantId from URL
    const wantId = extractWantId(request.url);
    
    if (!wantId) {
      return new Response(
        JSON.stringify({ success: false, error: 'Invalid wishlist item ID format' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    switch (request.method) {
      case 'PUT':
        return handleUpdateWishlistItem(request, wantId);
      case 'DELETE':
        return handleDeleteWishlistItem(request, wantId);
      default:
        return new Response(
    ...
    console.error('API Error in /api/wishlist/[wantId]:', error);
    return new Response(
      JSON.stringify({ success: false, error: 'Internal server error' }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
